...
import { setShowSendPage, setShouldShowBottomNav, 
  setusdcSolValue, setusdtSolValue,
  seteurcSolValue, } from '../../redux/userWalletData.tsx';
      ...
      if (isNaN(amountToNumber)) {
        ...
        if (sendingToSelectedContact) {
          ...
          if (selectedLanguageCode == 'es') {
            ...
            setErrorMessage('Sorry, there was an error with your transaction. Please try again.')
          }
        }
        } else {
        ...
        */
        ...
        if (locatedUser) {
          ...
          for (const linkedAccount of locatedUser.linked_accounts) {
            if (linkedAccount['type'] == 'wallet' && isValidSolanaAddress(linkedAccount['address'])) {
          ...
          receiverPublicKey = await pregenerateUserAndReturnPublicKey(cleanedAddress); // Take an email and make a new user
        }
...
async function pregenerateUserAndReturnPublicKey(emailAddress) {
  try {
    const pregenerateUserFn = httpsCallable(functions, "pregeneratePrivyUser");
  ...
  if (users) {
    ...
    for (const user of users) {
      ...
      for (const linkedAccount of user.linked_accounts) {
        console.log("checking linkedAccount['address']", linkedAccount['address'])
