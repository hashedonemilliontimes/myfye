...
export default function Header({ languageProps, colorProps, pageProps }: HeaderProps) {
  ...
  const [scrollingOpacity, setScrollingOpacity] = useState(0);
  ...
  useEffect(() => {
    function handleScroll() {
      ...
      const opacity = Math.min(scrollY / 400, 1); // Grow from 0 to 1 as scrollY increases

      setScrollingOpacity(opacity);
    }
...
>
      ...
      >
        ...
        src={ pageProps.page === 'dashboard' ? (walletConnected ? leafIcon : leafIcon) : leafIcon} style={{width: '40px', height: 'auto', 
        marginRight: '15px', cursor: 'pointer', 
        opacity: pageProps.page === 'home' ? scrollingOpacity : 1}} alt="MyFye" 
        /> 
        { pageProps.page != 'home' && (
...
<div
        ...
        {/*globeVisible && ( <GlobeMenu languageProps = {{ language: languageProps.language}} colorProps = {{color: colorProps.color}}/>)*/}
              ...
              <span>
                ...
                onClick={() => {
                  navigate(`/${languageProps.language}/app`);
                  setNavMenuVisible(false); // Close the menu when a link is clicked
                  window.scrollTo(0, 0);
                }}>
              <img
