      ...
      const handleDepositButtonClick = async () => {
        if (depositButtonActive) {
          ...
          if (isNaN(depositToNumber)) {
            ...
            const depositSuccess: boolean = await requestNewSolanaTransaction(cryptoList[0].address, convertToSmallestDenomination, 
              'usdcSol', null, currentValue, principalHistory, dispatch, "Phantom");
            ...
            if (depositSuccess) {
              ...
              const swapDepositorSolanaStableCoinWithUsdy = httpsCallable(functions, 
                ...
                swapDepositorSolanaStableCoinWithUsdy({ depositorPubKey: publicKey, 
                  amountInUSD: depositToNumber, currencyType: 'solUsdc' })
                .then((result) => {
