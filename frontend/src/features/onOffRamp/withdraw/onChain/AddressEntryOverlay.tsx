...
import {
  saveSolAddress,
  getRecentSolAddresses,
  RecentSolAddress,
} from "@/functions/RecentSolAddress";
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.success(
          `Sent ${amount} ${selectedToken} to ${address.slice(
            0,
            6
          )}...${address.slice(-4)}`
        );
        ...
        >
          ...
          >
            ...
            <div
              ...
              `}
            >
              Sending <span style={{ fontWeight: "bold" }}>{amount}</span>{" "}
              <span style={{ fontWeight: "bold" }}>{selectedToken}</span>
            </div>
          </div>
