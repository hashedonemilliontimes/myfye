...
import { createPublicClient, createWalletClient, http, parseAbi, custom, hashTypedData, encodeFunctionData } from 'viem';
...
const WithdrawConfirmOverlay = ({
  ...
  const executePayOut = async () => {
    try {
      ...
      console.log("BlindPay contract address:", payoutQuote.contract.blindpayContractAddress);
      ...
      console.log("Using USDC EIP-2771 meta-transactions for gasless approval...");
      ...
      console.log("Using Base's sponsored transactions for gasless USDC approval...");
      ...
      try {
        ...
        console.log("Using Base sponsored transactions for gasless approval...");
        ...
        const baseRelayerResponse = await fetch('https://relayer.base.org/sponsored', {
          method: 'POST',
          headers: {
        ...
        if (!baseRelayerResponse.ok) {
          throw new Error(`Base relayer error: ${baseRelayerResponse.statusText}`);
        }
        ...
        console.log("Simulating Base sponsored transaction success for testing...");
