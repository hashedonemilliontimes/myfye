...
export const tokenTransfer = async (
  ...
  try {
    ...
    if (
      !amountSmallestDenomination ||
      typeof amountSmallestDenomination !== "number"
    ) {
    ...
    const payerParsedTokenAccounts =
    ...
    const payerAccountInfo = payerParsedTokenAccounts.value.find(
      (accountInfo: {
        account: { data: { parsed: { info: { mint: string } } } };
    ...
    const receiverParsedTokenAccounts =
      await connection.getParsedTokenAccountsByOwner(destinationPublicKey, {
        programId: new PublicKey(programId),
      });
    ...
    if (!receiverAccountInfo) {
      ...
      try {
        ...
        while (attempts < maxAttempts) {
          console.log(
            "Looking for newly created token account (Attempt " +
              (attempts + 1) +
              ")"
          );
        ...
        throw new Error(
          "Failed to create or fetch the token account: " + error
        );
      }
    } else {
      console.log(currencySelected + " account found for receiver.");
    ...
    const signedTx = Transaction.from(
      Buffer.from(responseData.signedTransaction, "base64")
    );
