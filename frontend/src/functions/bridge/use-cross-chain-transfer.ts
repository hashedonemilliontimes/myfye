...
export function useCrossChainTransfer() {
  ...
  const getEVMBalance = async (chainId: SupportedChainId, walletAddress: String ) => {
    ...
    console.log("Fetching getEVMBalance for:", walletAddress);
    ...
    console.log("Balance getevm fetched:", balance);
  ...
  const executeTransfer = async (
    ...
    try {
      ...
      await embeddedWallet.switchChain(sourceChainId);

      // If provided, use the embedded wallet client
      if (!isSourceSolana && embeddedWallet) {
        sourceClient = walletClient;
